...
//
// SENGAJA TIDAK ADA DATA SLA DI SINI (deadline, % SLA, prioritas).
// TV bisa dilihat orang di luar Logistik, jadi SLA tidak boleh ikut tampil.
...
function startOfWibMonth(d: Date): Date {
  const s = new Date(d.getTime() + WIB_OFFSET_MS);
  s.setUTCDate(1);
  s.setUTCHours(0, 0, 0, 0);
  return new Date(s.getTime() - WIB_OFFSET_MS);
...
export async function getTvData(): Promise<TvData> {
  ...
  // --- Ambil nama & inisial staf (hanya kolom yang aman) ---
  const userIds = [
    ...new Set([
  ...
  const users = userIds.length
    ? await db.user.findMany({ where: { id: { in: userIds } }, select: { id: true, name: true, initial: true } })
    : [];
