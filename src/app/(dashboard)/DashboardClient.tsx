...
import { Activity, Clock, CheckCircle, AlertCircle, Inbox, ArrowUpRight } from 'lucide-react';
...
export default function DashboardClient({ 
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Bento Grid Layout - Light Premium */}
      <motion.div 
      ...
      >
        ...
        <motion.div 
          ...
          className="md:col-span-2 lg:col-span-4 bg-white p-6 rounded-[24px] border border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-300 relative overflow-hidden group"
        >
        ...
        <div className="md:col-span-4 lg:col-span-4 grid grid-rows-2 gap-5">
          <motion.div 
            variants={itemVariants}
            className="bg-white p-5 rounded-[20px] border border-slate-200/60 shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] flex items-center justify-between group transition-all"
          >
            <div>
      ...
      {/* Workload Section - Light Sleek UI */}
      {userRole !== 'PIC_LOGISTIK' && (
        <motion.div 
          initial={{ opacity: 0, y: 20 }} 
          animate={{ opacity: 1, y: 0 }} 
          transition={{ duration: 0.6, delay: 0.3 }}
          className="bg-white p-6 md:p-8 rounded-[24px] border border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)]"
        >
          <div className="flex items-center gap-2 mb-6 border-b border-slate-100 pb-4">
            <div className="w-2 h-2 bg-indigo-500 rounded-full"></div>
            <h3 className="text-sm font-bold text-slate-700 uppercase tracking-widest">Beban Kerja PIC</h3>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {picWorkload.map((pic, idx) => (
              <motion.div 
                ...
                className="flex justify-between items-center p-4 bg-slate-50/50 rounded-xl border border-slate-100 hover:border-indigo-100 hover:shadow-sm transition-all duration-300"
              >
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-600 font-bold text-xs shadow-sm">
                    {pic.name.charAt(0)}
                  </div>
                  <span className="font-bold text-slate-700 text-sm">{pic.name}</span>
                </div>
                <div className="flex gap-2 text-[10px]">
                  <span className="text-slate-600 bg-white px-2.5 py-1.5 rounded-lg font-bold border border-slate-200 shadow-sm">{pic.activeTasks} Antrean</span>
                  <span className="text-emerald-700 bg-emerald-50 px-2.5 py-1.5 rounded-lg font-bold border border-emerald-100 shadow-sm">{pic.completed} Selesai</span>
                </div>
              </motion.div>
            ))}
          </div>
