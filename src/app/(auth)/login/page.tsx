...
import { Lock, ArrowRight, Loader2, AlertCircle, UserCircle } from 'lucide-react';
...
export default function LoginPage() {
  ...
  // State untuk initial dan password
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative group">
                <UserCircle className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-600 transition-colors duration-300" size={18} />
                <input 
                  type="text" required placeholder="IND" 
                  value={initial} onChange={(e) => setInitial(e.target.value.toUpperCase())}
                  maxLength={3}
                  className="w-full pl-11 pr-4 py-3.5 bg-slate-50 border border-slate-200/60 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-300 transition-all duration-300 text-sm text-slate-800 font-bold placeholder:text-slate-400 placeholder:font-medium shadow-sm uppercase"
                />
              </div>
            </div>
